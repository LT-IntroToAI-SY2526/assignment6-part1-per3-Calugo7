...
def visualize_results(X_train, y_train, X_test, y_test, predictions, model):
    ...
    plt.scatter(X_test, predictions, color='red', marker='x', s=100, alpha=0.7, label='Predictions')
    ...
    plt.xlabel('Hours Studied', fontsize=12)
    plt.ylabel('Test Scores', fontsize=12)
    plt.title('Linear Regression: Student Performance Prediction', fontsize=14, fontweight='bold')
    plt.legend()
    plt.grid(True, alpha=0.3)
...
def make_prediction(model, hours):
    ...
    prediction = model.predict(hour_array)[0]

    print("\n=== New Prediction ===")
    print(f"If a student studies {hours} hours, predicted score: {prediction:.2f}")
